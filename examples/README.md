...
```shell
cargo run -- --provider inspire --check --output examples/generated-inspire.bib examples/inspire-main.tex
```

Update an existing BibTeX file in place:

```shell
cargo run -- --provider inspire --force-regenerate --no-backup examples/generated-inspire.bib
```
...
```shell
cargo run -- --provider auto --output examples/generated-auto.bib --no-backup examples/inspire-main.tex
```
